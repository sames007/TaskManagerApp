...
public class DatabaseManager {
    ...
    public UserSession getAccount(String username) {
        try (PreparedStatement stmt = getConnection().prepareStatement("SELECT * FROM users WHERE UserName = ?")) {
            stmt.setString(1, username);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
    ...
    public void addTask(@NotNull Task task) {
        try (PreparedStatement stmt = conn.prepareStatement("INSERT INTO Tasks (Description, DueDate, DueTime, FK_PriorityID, FK_CategoryID, FK_UserID, Status) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
            ...
            stmt.setInt(6, task.getUserID()); // Use the task's user ID
    ...
    public void loadTasks(ObservableList<Task> tasks) {
        if (tasks.isEmpty()) {
            System.out.println("No tasks list provided or empty list");
            return;
        }

        try (PreparedStatement stmt = conn.prepareStatement("SELECT * FROM Tasks WHERE FK_UserID = ?")) {
            stmt.setInt(1, tasks.get(0).getUserID()); // Get user ID from the first task in the list
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    Task task = new Task(
                    ...
                    task.setUserID(rs.getInt("FK_UserID")); // Set the user ID for the task
                    tasks.add(task);
                    System.out.println("Task added to list: " + task.getDescription());
                }
            }
        } catch (SQLException e) {
    ...
    // Overloaded method to retrieve all tasks for a specific user from the database
    public void loadTasks(int userID, ObservableList<Task> tasks) {
        if (tasks == null) {
            System.out.println("No tasks list provided");
            return;
        }
        try (PreparedStatement stmt = getConnection().prepareStatement("SELECT * FROM Tasks WHERE FK_UserID = ?")) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    task.setStatus(rs.getString("Status"));
                    task.setTaskID(rs.getInt("TaskID"));
                    task.setUserID(rs.getInt("FK_UserID"));
                    tasks.add(task);
                    System.out.println("Task added to list: " + task.getDescription());
                }
            }
